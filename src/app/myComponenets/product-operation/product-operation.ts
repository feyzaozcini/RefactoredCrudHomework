...
import { createProductForm,toCreateProductRequest } from '../../Valications/Products/CreateProductFormFactory';
import { updateProductForm,toUpdateProductRequest } from '../../Valications/Products/UpdateProductFormFactory';
...
export class ProductOperation implements OnInit {
  ...
  private async refreshProducts():Promise<void>{
    try {
      ...
      console.log("Ürün listesi alinamadi ",error);
    }
  }
  ...
  private async refreshCategories():Promise<void>{
    try {
      ...
      console.log("Kategori listesi alinamadi ",error);
    }
  }
  ...
  protected getErrorMessage(control:AbstractControl|null, label='Bu alan'):string|null{
    if(!control || (!control.touched && !control.dirty) || !control.invalid) return null;
      ...
      return `${label} en az ${e.min} olmalidir`;
    }
